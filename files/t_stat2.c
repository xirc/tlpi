...
static char*
dtime(const struct timespec *timep) {
    static char s1[1024];
    static char s2[1024];
    struct tm *t;

    t = gmtime(&timep->tv_sec);
    if (t == NULL) {
        return NULL;
    }
    if (strftime(s1, 1024, "%F %T.%%06u", t) == 0) {
        return NULL;
    }

    sprintf(s2, s1, timep->tv_nsec);
    return s2;
}
...
{
    struct stat sb;
    int stat_link;      /* True if "-l" specified (i.e., use lstat) */
