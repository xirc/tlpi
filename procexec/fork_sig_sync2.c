     ...
     char *argv[] __attribute__((unused)))
{
    ...
    case 0:
        ...
        /* And then child waits for parent to
        ...
        printf("[%s %ld] Child got signal\n",
                now("%T"), (long) getpid());
        /* If requied, return signal mask to its original state */
    ...
    default:
        ...
        /* Parent does some required action here... */
        printf("[%s %ld] Parent doing some work\n",
                now("%T"), (long) getpid());
        sleep(2);
            /* Simulate time spent doing some work */
        /* And then signals child that is's done */
        printf("[%s %ld] Parent about to signal child\n",
                now("%T"), (long) getpid());
        if (kill(cpid, SYNC_SIG) == -1) {
            perror("kill");
            exit(EXIT_FAILURE);
        }
